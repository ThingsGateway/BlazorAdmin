...
internal sealed class SysResourceService : BaseService<SysResource>, ISysResourceService
{
    ...
    {
        var sysResources = App.GetService<ICacheService>().Cache.Get<List<SysResource>>(CacheKey);
        if (sysResources == null)
        {
            using var db = GetDB();
            sysResources = await db.Queryable<SysResource>().ToListAsync().ConfigureAwait(false);
            App.GetService<ICacheService>().Cache.Set(CacheKey, sysResources);
        }
        return sysResources;
    }
    ...
    {
        App.GetService<ICacheService>().Cache.Remove(CacheKey);
        //删除超级管理员的缓存
