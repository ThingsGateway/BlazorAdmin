...
public class AuthService : IAuthService
{
    ...
    public async Task<LoginOutput> LoginAsync(LoginInput input, bool isCookie = true)
    {
        ...
        if (isCookie) //openApi登录不再需要解密
        {
            try
            {
                password = DESDataEncryption.Decrypt(input.Password);//解密
            }
            catch (Exception)
            {
    ...
    private async Task BeforeLoginAsync(AppConfig appConfig, LoginInput input)
    {
        ...
        var errorCountCache = App.GetService<ICacheService>().Cache.Get<int>(key);//获取登录错误次数

        if (errorCountCache >= appConfig.LoginPolicy.ErrorCount)
        {
            App.GetService<ICacheService>().Cache.SetExpire(key, TimeSpan.FromMinutes(appConfig.LoginPolicy.ErrorLockTime));//设置缓存
    ...
    private void LoginError(LoginPolicy loginPolicy, string userName)
    {
        ...
        App.GetService<ICacheService>().Cache.Increment(key, 1);// 登录错误次数+1
        App.GetService<ICacheService>().Cache.SetExpire(key, TimeSpan.FromMinutes(loginPolicy.ErrorResetTime));//设置过期时间
        var errorCountCache = App.GetService<ICacheService>().Cache.Get<int>(key);//获取登录错误次数
    ...
    private async Task UpdateUser(LoginEvent loginEvent)
    {
        ...
        App.GetService<ICacheService>().Cache.Remove(key);//移除登录错误次数
        ...
        if (await db.Updateable(sysUser).UpdateColumns(it => new
        {
            ...
            App.GetService<ICacheService>().Cache.HashAdd(CacheConst.Cache_SysUser, sysUser.Id.ToString(), sysUser);//更新Cache信息
    }
