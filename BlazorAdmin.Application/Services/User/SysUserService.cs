...
using BlazorAdmin.Json.Extension;
...
internal sealed class SysUserService : BaseService<SysUser>, ISysUserService
{
    ...
    public async Task<bool> CheckApiDataScopeAsync(long? orgId, long createUerId, bool throwEnable = true)
    {
        ...
        {
            throw Oops.Bah(App.GetService<IStringLocalizerFactory>().Create(typeof(BlazorAdmin.Application.OperDescAttribute))["NoPermission"]);
        }
    ...
    {
        //先从Cache拿
        var sysUser = App.GetService<ICacheService>().Cache.HashGetOne<SysUser>(CacheConst.Cache_SysUser, userId.ToString());
    ...
    public async Task<long> GetIdByAccountAsync(string account, long? tenantId)
    {
        ...
        //先从Cache拿
        var userId = App.GetService<ICacheService>().Cache.HashGetOne<long>(key, account);
        if (userId == 0)
        {
            ...
            {
                //插入Cache
                App.GetService<ICacheService>().Cache.HashAdd(key, account, userId);
            }
        }
        return userId;
    }
...
#pragma warning disable CS8625 // 无法将 null 字面量转换为非 null 的引用类型。
                ...
                u.Phone = DESDataEncryption.Decrypt(u.Phone);//解密手机号
...
#pragma warning restore CS8625 // 无法将 null 字面量转换为非 null 的引用类型。
    ...
    public void DeleteUserFromCache(IEnumerable<long> ids)
    {
        ...
        var sysUsers = App.GetService<ICacheService>().Cache.HashGet<SysUser>(CacheConst.Cache_SysUser, userIds).Where(it => it != null);//获取用户列表
        if (sysUsers.Any() == true)
        {
            ...
            if (sysUsers.Any(it => it.TenantId != null))//如果有租户id不是空的表示是多租户模式
            {
                ...
                foreach (var tenantId in tenantIds)
                {
                    ...
                    //删除账号
                    App.GetService<ICacheService>().Cache.HashDel<long>(userAccountKey, accounts);
                }
            }
            //删除用户信息
            App.GetService<ICacheService>().Cache.HashDel<SysUser>(CacheConst.Cache_SysUser, userIds);
            //删除账号
            App.GetService<ICacheService>().Cache.HashDel<long>(CacheConst.Cache_SysUserAccount, accounts);

            App.GetService<ICacheService>().Cache.HashDel<VerificatInfo>(CacheConst.Cache_Token, userIds.Select(it => it.ToString()).ToArray());
    ...
    private async Task<string> GetDefaultPassWord(bool isEncrypt = false)
    {
        ...
        return isEncrypt ? DESDataEncryption.Encrypt(appConfig.PasswordPolicy.DefaultPassword) : appConfig.PasswordPolicy.DefaultPassword;//判断是否需要加密
    }
    ...
    private async Task CheckInput(SysUser sysUser)
    {
        ...
        if (!string.IsNullOrEmpty(sysUser.Phone))
        {
            ...
            sysUser.Phone = DESDataEncryption.Encrypt(sysUser.Phone);
        }
    ...
    private async Task<SysUser?> GetUserFromDbAsync(long userId)
    {
        ...
        if (sysUser != null)
        {
            sysUser.Password = DESDataEncryption.Decrypt(sysUser.Password);//解密密码
            sysUser.Phone = DESDataEncryption.Decrypt(sysUser.Phone);//解密手机号
            ...
            //插入Cache
            App.GetService<ICacheService>().Cache.HashAdd(CacheConst.Cache_SysUserAccount, sysUser.Account, sysUser.Id);
            App.GetService<ICacheService>().Cache.HashAdd(CacheConst.Cache_SysUser, sysUser.Id.ToString(), sysUser);
