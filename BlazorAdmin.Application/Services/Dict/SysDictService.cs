...
internal sealed class SysDictService : BaseService<SysDict>, ISysDictService
{
    ...
    public async Task<AppConfig> GetAppConfigAsync()
    {
        ...
        var appConfig = App.GetService<ICacheService>().Cache.Get<AppConfig>(key);
        if (appConfig == null)
        {
            ...
            App.GetService<ICacheService>().Cache.Set(key, appConfig);
        }
    ...
    public async Task<SysDict> GetByKeyAsync(string category, string name)
    {
        ...
        var sysDict = App.GetService<ICacheService>().Cache.HashGetOne<SysDict>(key, field);
        if (sysDict == null)
        {
            using var db = GetDB();
            ...
            App.GetService<ICacheService>().Cache.HashAdd(key, field, sysDict);
        }
        return sysDict;
    }
    ...
    public async Task<IDictionary<string, SysDict>> GetDefineConfigAsync()
    {
        ...
        var sysDicts = App.GetService<ICacheService>().Cache.HashGetAll<SysDict>(key);
        if (sysDicts.Count == 0)
        {
            using var db = GetDB();
            ...
            App.GetService<ICacheService>().Cache.Set(key, sysDicts);
        }
    ...
    public async Task<List<SysDict>> GetSystemConfigAsync()
    {
        ...
        var sysDicts = App.GetService<ICacheService>().Cache.Get<List<SysDict>>(key);
        if (sysDicts == null)
        {
            using var db = GetDB();
    ...
    {
        App.GetService<ICacheService>().Cache.Remove($"{CacheConst.Cache_SysDict}{define}");
        if (define == DictTypeEnum.System)
            App.GetService<ICacheService>().Cache.Remove($"{CacheConst.Cache_SysDict}{define}{nameof(AppConfig)}");
    }
