...
internal sealed class SysOrgService : BaseService<SysOrg>, ISysOrgService
{
    ...
    public async Task<List<SysOrg>> GetAllAsync(bool showDisabled = true)
    {
        ...
        var sysOrgs = App.GetService<ICacheService>().Cache.Get<List<SysOrg>>(key);
        if (sysOrgs == null)
        {
            using var db = GetDB();
            sysOrgs = (await db.Queryable<SysOrg>().ToListAsync().ConfigureAwait(false));
            App.GetService<ICacheService>().Cache.Set(key, sysOrgs);
        }
        if (!showDisabled)
        {
    ...
    public async Task<List<SysOrg>> GetTenantListAsync()
    {
        ...
        var tenantList = App.GetService<ICacheService>().Cache.Get<List<SysOrg>>(key);
        if (tenantList == null)
        {
            ...
            {
                //插入Redis
                App.GetService<ICacheService>().Cache.Set(key, tenantList);
            }
        }
        return tenantList;
    }
    ...
    public async Task<long?> GetTenantIdByOrgIdAsync(long orgId, List<SysOrg> sysOrgList = null)
    {
        ...
        //先从缓存拿租户Id
        var tenantId = App.GetService<ICacheService>().Cache.HashGetOne<long?>(key, orgId.ToString());
        if (tenantId == null)
        {
            //获取所有组织
            sysOrgList ??= await GetAllAsync().ConfigureAwait(false);
            ...
            if (userOrg != null)
            {
                ...
                if (tenantId != null)
                    App.GetService<ICacheService>().Cache.HashAdd(key, orgId.ToString(), tenantId);//插入缓存
            }
        }
        return tenantId;
    }
    ...
    {
        App.GetService<ICacheService>().Cache.Remove($"{CacheConst.Cache_SysOrg}");
        App.GetService<ICacheService>().Cache.Remove($"{CacheConst.Cache_SysUser}");
        App.GetService<ICacheService>().Cache.Remove($"{CacheConst.Cache_SysTenant}");
        App.GetService<ICacheService>().Cache.Remove($"{CacheConst.Cache_SysOrgTenant}");
