...
internal sealed class SysRoleService : BaseService<SysRole>, ISysRoleService
{
    ...
    public async Task<List<SysRole>> GetAllAsync()
    {
        ...
        var sysRoles = App.GetService<ICacheService>().Cache.Get<List<SysRole>>(key);
        if (sysRoles == null)
        {
            using var db = GetDB();
            sysRoles = await db.Queryable<SysRole>().ToListAsync().ConfigureAwait(false);
            App.GetService<ICacheService>().Cache.Set(key, sysRoles);
        }
        return sysRoles;
    }
    ...
    {
        App.GetService<ICacheService>().Cache.Remove(CacheConst.Cache_SysRole);//删除KEY
