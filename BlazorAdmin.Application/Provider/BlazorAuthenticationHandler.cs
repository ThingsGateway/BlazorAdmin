...
public class BlazorAuthenticationHandler : AppAuthorizeHandler
{
    ...
    static async Task Fail(AuthorizationHandlerContext context)
    {
        ...
        if (verificatInfo != null)
            await App.GetService<INoticeService>().UserLoginOut(verificatInfo.ClientIds, App.GetService<IStringLocalizerFactory>().Create(typeof(BlazorAuthenticationHandler))["UserExpire"]).ConfigureAwait(false);
