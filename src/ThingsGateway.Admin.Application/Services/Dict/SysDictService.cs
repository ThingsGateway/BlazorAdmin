...
internal class SysDictService : BaseService<SysDict>, ISysDictService
{
    ...
    public async Task EditPagePolicyAsync(PagePolicy input)
    {
        ...
        List<SysDict> dicts = new List<SysDict>()
        {
            new SysDict() { DictType = DictTypeEnum.System, Category = nameof(PagePolicy), Name = nameof(PagePolicy.DefaultShortcuts), Code = input.DefaultShortcuts.ToJsonNetString() },
    ...
    public async Task<AppConfig> GetAppConfigAsync()
    {
        ...
        if (appConfig == null)
        {
            ...
            appConfig.PagePolicy.DefaultShortcuts = sysDicts.FirstOrDefault(a => a.Category == nameof(PagePolicy) && a.Name == nameof(PagePolicy.DefaultShortcuts))?.Code.FromJsonNetString<List<long>>() ?? new List<long>();
