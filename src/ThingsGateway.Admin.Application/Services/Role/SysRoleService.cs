...
class SysRoleService : BaseService<SysRole>, ISysRoleService
{
    ...
    public async Task<GrantResourceData> OwnResourceAsync(long id, RelationCategoryEnum category = RelationCategoryEnum.RoleHasResource)
    {
        ...
        roleOwnResource.GrantInfoList = relations.Select(it => (it.ExtJson?.FromJsonNetString<long>() ?? 0)).Where(a => a != 0);
    ...
    public async Task<GrantPermissionData> ApiOwnPermissionAsync(long id)
    {
        ...
        roleOwnPermission.GrantInfoList = relations.Select(it => it.ExtJson?.FromJsonNetString<RelationRolePermission>()!).Where(a => a != null);
