...
public partial class ParentOrg
{
    [Inject]
    [NotNull]
    ...
    [Inject]
    [NotNull]
    public IStringLocalizer<ParentOrg>? Localizer { get; set; }
    ...
    private async Task OnSelect()
    {
        Option = new DialogOption()
        {
            ...
            Title = Localizer["Choice"],
            BodyTemplate = BootstrapDynamicComponent.CreateComponent<ParentOrgTree>(new Dictionary<string, object?>
            {
                [nameof(ParentOrgTree.Value)] = Value,
                [nameof(ParentOrgTree.ValueChanged)] = EventCallback.Factory.Create<long>(this, v => OnValueChanged(v))
            }).Render(),
            FooterTemplate =
