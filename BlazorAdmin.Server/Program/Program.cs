        ...
        await Serve.RunAsync(RunOptions.Default.ConfigureBuilder(builder =>
        {
            ...
            .Configure(app =>
            {
                ...
                app.MapRazorComponents<BlazorApp>()
                    .AddAdditionalAssemblies(App.EffectiveTypes.Where(u => !u.IsInterface && !u.IsAbstract && u.IsClass
&& u.IsDefined(typeof(Microsoft.AspNetCore.Components.RouteAttribute), true)).Select(a => a.Assembly).Distinct().ToList().Where(a => a != typeof(Program).Assembly).ToArray())
