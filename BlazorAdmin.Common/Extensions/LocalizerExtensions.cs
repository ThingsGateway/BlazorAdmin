...
public static class LocalizerExtensions
{
    ...
    public static string GetMethodDisplayName(this Type modelType, string methodName)
    {
        ...
        var displayName = App.GetService<ICacheService>().Cache.GetOrAdd(cacheKey, entry =>
        {
            string? dn = null;
            // 显示名称为空时通过资源文件查找 methodName 项
            var localizer = modelType.Assembly.IsDynamic ? null : App.GetService<IStringLocalizerFactory>().Create(modelType);
    ...
    private static bool TryGetProperty(Type modelType, string fieldName, [NotNullWhen(true)] out PropertyInfo? propertyInfo)
    {
        ...
        propertyInfo = App.GetService<ICacheService>().Cache.GetOrAdd(cacheKey, entry =>
        {
    ...
    private static void ValidateDataAnnotations(object? value, ValidationContext context, List<ValidationResult> results, PropertyInfo propertyInfo, string? memberName = null)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            if (result != null && result != ValidationResult.Success)
            {
                ...
                //    && App.GetService<IStringLocalizerFactory>().Create(context.ObjectType).TryGetLocalizerString(rule.ErrorMessage, out var msg))
                //{
                ...
                //    && App.GetService<IStringLocalizerFactory>().Create(rule.GetType()).TryGetLocalizerString(nameof(rule.ErrorMessage), out msg))
                //{
                //    rule.ErrorMessage = msg;
                //    find = true;
                //}
                ...
                // 通过 字段.规则名称 检索
                if (!context.ObjectType.Assembly.IsDynamic && !find
                    && App.GetService<IStringLocalizerFactory>().Create(context.ObjectType).TryGetLocalizerString($"{memberName}.{ruleName.ToString()}", out var msg))
                {
                    rule.ErrorMessage = msg;
                    find = true;
                }
